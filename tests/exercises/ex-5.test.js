...
describe("exercise1", () => {
    ...
    it('Should update any the "healthy" column of any dolphin for any brown or green dolphin to FALSE', async (done) => {
        ...
        await testUtils.safeExpect(expect, healthyCount, 3, "Unexpected number of healthy dolphins! Only update the 'healthy' to FALSE for dolphins that are either brown OR green")

        let d2 = result.find(d => d.name === "d2")
        let d4 = result.find(d => d.name === "d4")

        await testUtils.safeExpect(expect, d2.healthy, 0, "Found a green dolphin tagged as healthy - should be false!")
        await testUtils.safeExpect(expect, d4.healthy, 0, "Found a brown dolphin tagged as healthy - should be false!")
