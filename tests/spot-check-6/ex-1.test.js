...
describe("exercise1", () => {
    ...
    it('Should update every deity whose coolness level is above 10 to have a coolness level of 10', async (done) => {
        ...
        for(let r of result){
            await testUtils.safeExpect(expect, expectedLevels.includes(r.coolness) && r.coolness <= 10, true, "Only a deity whose coolness is above 10 should have their coolness changed to 10. Everyone else should stay the same.")
        }
